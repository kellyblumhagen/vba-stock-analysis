Sub stock_analysis():
    ...
    For Each ws In Worksheets
        ...
        For i = 2 To rowCount
            ...
            ' Print results if ticker changes
            If ws.Cells(i + 1, 1).Value <> ws.Cells(i, 1).Value Then
                ...
                Else
                    ...
                    ' Print results to separate worksheet
                    ws.Range("I" & 2 + j).Value = ws.Cells(i, 1).Value
                    ws.Range("J" & 2 + j).Value = Round(change, 2)
                    ws.Range("K" & 2 + j).Value = "%" & percentChange
                    ws.Range("L" & 2 + j).Value = total

                    ' Make positives green and negatives red
                    Select Case change
                        Case Is > 0
            ...
            ' If ticker stayed the same, add results
            Else
                total = total + ws.Cells(i, 7).Value
        ...
        ' Print max and min in separate columns
